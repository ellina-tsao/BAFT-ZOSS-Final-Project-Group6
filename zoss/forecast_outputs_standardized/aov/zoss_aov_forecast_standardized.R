# ============================================================
# ZOSS AOV Forecasting - Standardized Output Version
#
# 0. Setup
# 1. Load Data
...
output_dir <- file.path(output_root, "aov")
...
target_name <- "Average Order Value"
target_file_stem <- "aov"
target_business_meaning <- "Customer spending baseline and revenue planning."

final_model_internal <- "ARIMA_Selected"
final_model_name <- "ARIMA"
final_model_notation <- "ARIMA(1,0,1)(1,1,0)[12]"
final_model_reason <- "The original AOV script selected the explicit seasonal ARIMA after comparing the 2024 validation plot and error matrix."
...
model_reasons <- c(
  ...
  "ARIMA_Selected" = "Original final AOV model: explicit seasonal ARIMA used for 2025 forecasting.",
  "TSLM" = "Trend and monthly seasonality regression benchmark.",
  "NNet" = "Neural-network autoregression included in the original AOV script."
)

# Presentation comparison requested for AOV: show ARIMA, ETS, and NNet.
# Base ARIMA and SNaive remain in the full metrics table and appendix plot.
presentation_models_internal <- c(final_model_internal, "ETS", "NNet")
all_candidate_models_internal <- names(model_display)
y_labeler <- scales::label_dollar(prefix = "NT$", accuracy = 1, big.mark = ",")
...
# Keep the same AOV cleaning convention as the group script.
monthly_kpi <- required_data %>%
  filter(!is.na(Year), !is.na(Month)) %>%
  group_by(Year, Month) %>%
  summarise(
    AOV = mean(Total_Amount, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(Year, Month) %>%
  mutate(
    Month_Index = yearmonth(paste(Year, Month, sep = "-")),
    AOV_clean = as.numeric(tsclean(ts(AOV, start = c(2018, 7), frequency = 12))),
    Month_Date = as.Date(Month_Index),
    Value = AOV_clean
  ) %>%
  filter(Month_Index >= training_start, Month_Index <= validation_end) %>%
  select(Month_Index, Month_Date, AOV, Value) %>%
...
# No external calendar predictor is used for AOV. The only adjustment is the
# original tsclean() treatment retained from the group member script.
...
# Presentation request: show ARIMA, ETS, and NNet in the top-panel-only AOV
# comparison. Base ARIMA remains in the metrics table but is not shown here.
top3_models_internal <- c(final_model_internal, "ETS", "NNet")
...
# Preserve the original selected AOV model rather than switching purely by rank.
...
validation_all_candidate_plot <- plot_validation_comparison(
  ...
  y_labeler = y_labeler,
  title = "Average Order Value: Candidate Model Validation",
  subtitle = "Full appendix view of all original AOV candidate models.",
  validation_start = validation_start,
  validation_end = validation_end
)
...
final_forecast_plot <- plot_final_forecast(
  ...
  y_labeler = y_labeler,
  y_axis_title = "Average order value",
  future_start = future_start,
  future_end = future_end,
...
write.csv(
  ...
  file.path(output_dir, "aov_model_results.csv"),
  row.names = FALSE
)

write.csv(
  slide_ready_metrics,
  file.path(output_dir, "aov_model_results_slide_ready.csv"),
  row.names = FALSE
)

write.csv(
  final_forecast_tbl,
  file.path(output_dir, "aov_2025_final_forecast.csv"),
  row.names = FALSE
)
...
model_report <- c(
  ...
  "",
  "Models compared: Naive, SNaive, SES, ETS, Base ARIMA, Selected ARIMA, TSLM, NNet",
  "",
  ...
  "",
  "Notes / caveats:",
  "- AOV uses the original tsclean() cleaned series from the group member script.",
...
writeLines(model_report, file.path(output_dir, "aov_model_report.txt"))

ggsave(file.path(output_dir, "aov_validation_selected_models.png"), validation_selected_plot, width = 11, height = 8.5, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "aov_validation_selected_models_actual_forecast_only.png"), actual_forecast_plot, width = 11, height = 6.5, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "aov_validation_top3_actual_forecast_only.png"), top3_actual_forecast_plot, width = 11, height = 6.5, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "aov_validation_errors.png"), error_plot, width = 11, height = 5.8, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "aov_validation_all_candidate_models.png"), validation_all_candidate_plot, width = 12, height = 6.5, dpi = 300, bg = "white")
ggsave(file.path(output_dir, "aov_2025_final_forecast.png"), final_forecast_plot, width = 11, height = 6.5, dpi = 300, bg = "white")

print("=== AOV STANDARDIZED MODEL RESULTS ===")
